...
import { selectsColoresPacks, selectsPackSeleccionado } from "@/db/selects";
...
export default async function PackConcreto({ params }: { params: {modelo: string} }) {
  ...
  const [packSeleccionado3, colores] = await Promise.all([promisePackSeleccionado, promiseColores])
  
  return (
    <main className="flex flex-col items-center gap-10">
        <CompClientePack packSeleccionado={packSeleccionado3} colores={colores} /> 
    </main>
  );
